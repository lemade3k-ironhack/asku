...
router.post("/groups/:groupId/movies/create", validateEmpty, (req, res) => {
  const { title, plot, genre, director, image, trailer } = req.body
  const groupId = req.params.groupId;
  Movie.create({ title, plot, genre, director, image, trailer, _group: groupId })
    .then(movie => {
      Group.findByIdAndUpdate( groupId, { $push: {movies: movie._id }})
